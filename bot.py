...
#=========== ФУНКЦИЯ ВЫДАЧИ ЛУННОГО ЦИКЛА ============
    ...
    """Команда /moon — фаза луны на сегодня"""
    try:
        tz = get_tz()
        now_local = datetime.now(tz)
        moon = get_moon_phase(now_local)

        msg = f"Сегодня в {BOT_LOCATION['city']}е:\n{format_moon_phrase(moon)}\nВозраст: {moon['age_days']} суток"
        await update.effective_message.reply_text(msg)
    except Exception as e:
        logger.error(f"Ошибка /moon: {e}")
# ========== ВИКИПЕДИЯ - СЕРВИС ==========
